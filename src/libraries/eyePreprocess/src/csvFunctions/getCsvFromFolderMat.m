...
function getCsvFromFolderMat(foldermat, opts)

arguments
    foldermat string {mustBeFolder}
    opts.csvfolder = "data/csv"
    opts.exprSubj = "(\w+)"
    opts.ROI = []
...
allFilenames = arrayfun(@(x)string(x.name), dir(fullfile(foldermat, "*.mat")));
...
parfor s = 1:nSubj
    % determine how many
    try
    thisSubjectFilenames = H{s};
    nRuns = length(thisSubjectFilenames);
    timeZero = datetime;
    % special treatment if multiple runs
    if nRuns > 1
        o_ = cell(nRuns,1);
    end
    for cRun = 1:length(thisSubjectFilenames)
        thisFilename = thisSubjectFilenames(cRun);
        thisdata = parload(fullfile(foldermat,thisFilename));
        if thisdata.timestamps_RawData_ms(end) == 0
            thisdata.timestamps_RawData_ms(end) = [];
            thisdata.gaze = structfun(@deleteend, thisdata.gaze, "UniformOutput",false);
        end
        if source == "eyelink"
        ...
        assert(sum(I1) == sum(I2), "error in timepoints in %s, probably double timepoint somewhere", thisFilename)
