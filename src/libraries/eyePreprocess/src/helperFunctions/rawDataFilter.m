function [valOut,speedFiltData,devFiltData] ...
    = rawDataFilter(t_ms,diaSamples,gazeSamples,rawFiltSettings)
...
if nargin==2||nargin==0
    ...
    %----------------------------------------------------------------------
    % Exclude out of screen components:
    rawFiltSettings.exclude_gazebased                   = false;
...
%% Step 1.5: Remove Samples based on Out-of-Bounds gaze:

% Remove pupil samples when gaze is out of screen bounds:
isValid = removeGazeBased(t_ms,gazeSamples,isValid,rawFiltSettings);
...
isValid = removeLoners_multiple(t_ms,isValid,filtSettings);
...
%==========================================================================
function isValid = removeGazeBased(t_ms,gazeSamples,isValid_In,filtSettings)
% Removes samples that are not withing the acceptable range.
%
%--------------------------------------------------------------------------
if filtSettings.exclude_gazebased
% Get and check the settings:screenX = 1280;

    screenX = 1280;
    screenY = 1024;
    pad = 36;

    isValid = isValid_In & ~(gazeSamples(:,1) < -pad | gazeSamples(:,1) > screenX+pad | gazeSamples(:,2) < -pad | gazeSamples(:,2) > screenY+pad);

    % Remove isolated samples:
    isValid = removeLoners_multiple(t_ms,isValid,filtSettings);

    % Feedback:
    printToConsole(4,'Gaze Filter: %i samples removed.\n'...
        ,sum(~isValid&isValid_In));
else
    isValid = isValid_In;
end
...
isValid_Out = removeLoners_multiple(t_ms,isValid_Out,filtSettings);
...
for passIndx = 1:Npasses
    ...
    % allow for more types
    isValid_Running = removeLoners_multiple(t_ms,isValid_Running,filtSettings);
    % Log loop vars:
...
%EDIT mad should not be 0, else filter does not work. Set minimum value to 0.5?
if mad == 0
    mad = 0.5;
end
...
function isValid_Running = removeLoners_multiple(t_ms,isValid_Running,filtSettings,doPlot)
if nargin < 4
    doPlot = false;
end
    islandFilter_islandSeperation_ms_ = filtSettings.islandFilter_islandSeperation_ms;
    islandFilter_minIslandWidth_ms_ = filtSettings.islandFilter_minIslandWidth_ms;
    for iStep = 1:numel(islandFilter_minIslandWidth_ms_)
        filtSettings.islandFilter_minIslandWidth_ms = islandFilter_minIslandWidth_ms_(iStep);
        filtSettings.islandFilter_islandSeperation_ms = islandFilter_islandSeperation_ms_(iStep);
        isValid_Running = removeLoners(t_ms,isValid_Running,filtSettings,doPlot);
    end
    filtSettings.islandFilter_minIslandWidth_ms = islandFilter_minIslandWidth_ms_;
    filtSettings.islandFilter_islandSeperation_ms = islandFilter_islandSeperation_ms_;
end
