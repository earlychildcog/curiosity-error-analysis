...
% binfilenames = { [char(mffFile) filesep 'signal1.bin'] [char(mffFile) filesep 'signal2.bin'] };
binfilenames = { [mffFile filesep 'signal1.bin'] [mffFile filesep 'signal2.bin'] };
if ~isfile(binfilenames{2}) binfilenames(2) = []; end
...
for iFile = 1:length(binfilenames)
    ...
    if ~isempty(signalResource)
        ...
        error(sprintf('Error: Can not open the signal resource from file: %s.\n', mffFile));
    end
